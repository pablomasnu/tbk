...
describe TBK::Commerce do
  ...
  context "default_commerce" do
    it "should be nil if no default is configured" do
      (expect(TBK.config).to receive(:commerce_id)).and_return nil
      expect(TBK::Commerce.default_commerce).to be_nil
    end

    it "should return a valid commerce if configured" do
      (expect(TBK.config).to receive(:commerce_id)).at_least(:once).and_return 12345
      (expect(TBK.config).to receive(:commerce_key)).at_least(:once).and_return TBK::Commerce::TEST_COMMERCE_KEY
